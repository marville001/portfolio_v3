...
const NewClient: NextPage = () => {
  ...
  const handleSaveClient: SubmitHandler<Inputs> = async (data: any) => {
    ...
    const notification = toast.loading("Saving Client!")
    setSaving(true)
    try {

      await clientsModel.createClient(newClient);
      setSaving(false)
      toast.success("Client Saved Successfully!", { id: notification })
      clearForm()
      ...
      <AdminWrapper>
        <div className="_shadow2 relative my-12 flex flex-col items-center rounded-2xl bg-white dark:bg-dim-dark  p-6">
          ...
          <div className="flex flex-1 justify-center dark:text-white">
            <h2 className="text-3xl font-bold">New Client</h2>
          </div>

          <form onSubmit={handleSubmit(handleSaveClient)} className="flex flex-col md:flex-row my-16 w-full">
